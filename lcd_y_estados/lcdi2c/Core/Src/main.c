...
#define BRILLO_lim 2000 //tendre que ir variando
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) {
	if (estado_actual != S3 && GPIO_Pin == GPIO_PIN_0) { //de momento
...
void Cambio_de_Estados(){
	...
	case S2:
		...
		break;
	case S3:
		if(nivel == NIVEL_MAX) estado_actual= S4; //igual que o mayor que?
		else estado_actual =S2;
		break;
	case S4:
		estado_actual=S0;
		break;
	...
	switch (estado_actual) {
		          ...
		          case S3:
		        	  ...
		        	  if(flag_motor_empieza==0){
		        		  ...
		        		  flag_motor_empieza=1; //con este flag juntamos la parte de dani
...
int main(void)
{
  ...
  //HAL_Delay(500); //otro delay supongo...
  lcd_init ();
  lcd_clear();
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  //anti bloqueo del adc por favor diosito
	  if (__HAL_ADC_GET_FLAG(&hadc1, ADC_FLAG_OVR)) {
	            __HAL_ADC_CLEAR_FLAG(&hadc1, ADC_FLAG_OVR); // Limpiamos el error
	            HAL_ADC_Stop_DMA(&hadc1);                   // Paramos
	            HAL_ADC_Start_DMA(&hadc1, (uint32_t*)&valor_adc, 1); // Reiniciamos
	        }
