...
typedef enum {
    S0, //"reposo"
    S1, //"Nombre y esperar"
	S2, //"3...2...1" y en caso de que antes se haya fallado o acertado (con una variable que pueda valer -1, 0 o 1) manda mensaje también
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) {
	if (GPIO_Pin == GPIO_PIN_0) {
	        ...
	        if (HAL_GetTick() - last_tick > 250) { //antirrebote del boton
...
void Leer_() {
    // Iniciamos conversión, esperamos un tiempo mínimo (10ms) y leemos
    HAL_ADC_Start(&hadc1);
    if (HAL_ADC_PollForConversion(&hadc1, 10) == HAL_OK) {
        ldr_value = HAL_ADC_GetValue(&hadc1);
    }
    HAL_ADC_Stop(&hadc1); // Opcional si no es modo continuo
}
...
void Cambio_de_Estados(){
	...
	case S2:
		...
		break;
	case S3:
		//aqui va un if con la cuenta de aciertos
		break;
	case S4:
		estado_actual=S0;
		break;
...
void Acciones_Estados(){
	...
	char buffer[20]; //para guardar texto
	...
	switch (estado_actual) {
		          ...
		          case S1:
		        	  ...
		        	  lcd_put_cur(0,0);
		        	  lcd_send_string("*.*.*Pepita*.*.*");
		        	  lcd_put_cur(1,0);
		        	  sprintf(buffer, "HS=%d", highscore); //para guardar en el buffer el highscore
		          ...
		          case S3:
		        	  ...
		        	  if(transcurrido < TIMESUP_MS){
		        		  fallo_acierto=1; //ns si aqui es la mejor idea
		        		  char buf[10];
		        		  ...
		        		  fallo_acierto=-1;
		        		  estado_actual = S2; //para la prueba, luego sera a s4!!
		        	  }
		          ...
		          case S4:
		        	  ...
		        	  lcd_clear();
		        	  lcd_put_cur(0,0);
		        	  lcd_send_string("Enhorabuena!");//tendre que cambiar la palabra
		        	  break;
		      }
}
...
/*
void Pedir_Nombre(){
	char nombre[10]={0}; //pongo mas caracteres para que no me den errores
	char buffer_uart[1]; //buffer que recibe letra a letra
	char* msg="\r\nIntroduce tu nombre (max 7 letras) y pulsa ENTER: ";
	int indice=0;
	...
	while(1){
		...
		if (HAL_UART_Receive(&huart2, (uint8_t*)buffer_uart, 1, 10) == HAL_OK) {
            char c = buffer_uart[0];
            ...
            if (c == '\r' || c == '\n') {
        		...
        		nombre[indice++] = c;
		    	HAL_UART_Transmit(&huart2, (uint8_t*)buffer_uart, 1, 10);
			}
		}

		//si el usuario pulsa el boton durante la operacion
		if (estado_actual == S0) return;
	}

	//mostrar el nombre en pantalla
	    lcd_clear();
	    if (strlen(nombre) < 8 && strlen(nombre) > 0) {
	        lcd_put_cur(0, 0);
	        lcd_send_string(nombre);
	    } else {
	        lcd_put_cur(0, 0);
	        lcd_send_string("Nombre muy largo");
	        lcd_put_cur(1, 0);
	        lcd_send_string("o vacio");
	    }
...
int main(void)
{
  ...
  lcd_init ();
  lcd_clear();
  static Estado_t estado_anterior = -1;
